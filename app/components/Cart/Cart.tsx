          ...
          {cart.map((item) => (
            <div className={styles.item} key={item.id}>
              <div className={styles.itemConainer}>
                  ...
                  <div className={styles.itemPriseList}>
                     <div className={styles.quantity}>{item.quantity}x</div>
                     <div className={styles.price}>@${item.price} </div>
                     <div className={styles.totalPrice}>${item.price 
                      * 
                       item.quantity}</div>
                     </div>
                   </div>
                 <div onClick={() => handleDelete(item.id)} className={styles.deleteContainer}>
                   <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10"><path fill="#CAAFA7" d="M8.375 9.375 5 6 1.625 9.375l-1-1L4 5 .625 1.625l1-1L5 4 8.375.625l1 1L6 5l3.375 3.375-1 1Z"/></svg>
                 </div>
             </div>
          ))}
          <div style={{ marginTop: "20px" }}>
            <strong className={styles.totalOrder}>Order Total
              <div>
                ${totalSum}
              </div>
            </strong>
